...
void *threadfunc(void *thread_param)
{
    ...
    thread_func_args->thread_complete_success = false; // Initialize to false
    ...
    if (lock_result != 0)
    {
        ERROR_LOG("pthread_mutex_lock failed: %d", lock_result);
        return thread_param;
    }
    ...
    if (unlock_result != 0)
    {
        ERROR_LOG("pthread_mutex_unlock failed: %d", lock_result);
        return thread_param;
    }
